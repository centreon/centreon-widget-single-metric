  ...
  stage('Unit tests // RPMs packaging // Sonar analysis') {
    ...
    'Debian bullseye packaging and signing': {
      node {
        ...
        sh 'docker run -i --entrypoint "/src/centreon-widget-single-metric/ci/scripts/widget-deb-package.sh" -w "/src" -v "$PWD:/src" -e "DISTRIB=bullseye" -e "VERSION=$VERSION" -e "RELEASE=$RELEASE" registry.centreon.com/centreon-debian11-dependencies:22.04'
